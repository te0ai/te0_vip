...
//可逆性暗号化モジュール
require_once('../data/class/sp_blowfishv1.php');
$crypt = new Crypt_Blowfish(_KEY_);
...
class Crypt{
    ...
    function encrypt($plainText,$type='')
    {
        ...
        if (extension_loaded('mcrypt')) {
			$cipherText = mcrypt_generic($this->_td, $plainText);
			
			//110926WIREDADD
			$cipherText = base64_encode($cipherText);//バイナリから文字列へ変換
			
			//120925WIREDADD
			if( $type == 'url' ) $cipherText = str_replace(array('/','+','='),array('-','_','!'),$cipherText);
    ...
    function decrypt($cipherText,$type='')
    {
		...
		//20120123「+」が「 」ポスト時変換されるのを戻す
		$cipherText = str_replace(' ','+',$cipherText);
		
		//120925WIREDADD
		if( $type == 'url' ) $cipherText = str_replace(array('-','_','!'),array('/','+','='),$cipherText);


        if (extension_loaded('mcrypt')) {
			
			//110926WIREDADD
			$cipherText = base64_decode($cipherText);//文字列からバイナリに変換
			$plainText = mdecrypt_generic($this->_td, $cipherText);
			$plainText = rtrim($plainText,"\0");//余計な文字を削除
            return $plainText;
        }
